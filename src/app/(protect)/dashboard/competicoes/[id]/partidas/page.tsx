...
export default async function PartidasCompeticaoPage(props: any) {
  ...
  const { data: partidas } = await (await supabase)
    ...
    .select(`
      *,
      equipe_a:equipes!equipe_a_id(nome, responsavel_email, responsavel_nome, modalidade),
      equipe_b:equipes!equipe_b_id(nome, responsavel_email, responsavel_nome, modalidade)
    `)
    .eq("competicao_id", params.id)
    .order("data", { ascending: true });

  const partidasPorModalidade: Record<string, any[]> = {};
  partidas?.forEach(partida => {
    const modality = partida.modalidade || partida.equipe_a?.modalidade || partida.equipe_b?.modalidade || "Sem modalidade";
    if (!partidasPorModalidade[modality]) {
      partidasPorModalidade[modality] = [];
    }
    partidasPorModalidade[modality].push(partida);
  });
  ...
  return (
    <div className="space-y-6 px-4 md:px-8 max-w-5xl mx-auto py-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        {Object.keys(partidasPorModalidade).length > 0 ? (
          Object.entries(partidasPorModalidade).map(([modalidade, listaPartidas]) => (
            <div key={modalidade} className="mb-8 last:mb-0">
                  ...
                  <div key={partida.id} className="border rounded-lg p-4 hover:shadow-sm transition">
                    ...
                    <div className="flex justify-between items-center mt-3">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <DeleteMatchButton matchId={String(partida.id)} />
                      </div>
                    </div>

                    {partida.status === 'pendente' && (
                      <form
                        action={updateMatchDateAndLocal} 
